...
public class OpenNISkeleton : Skeleton
{
	...
	public SkeletonJointTransformation[] JointData;
	public bool absolute = true;
	
	public Quaternion GetJointWorldRotation(SkeletonJoint joint)
	{
		return Quaternion.Inverse(transform.rotation) * transforms[(int)joint].rotation;
	}
	...
	public void UpdateJoint(SkeletonJoint joint, SkeletonJointTransformation skelTrans)
	{
		...
		/*
		//Clavicle correction
		if(joint == SkeletonJoint.RightShoulder)
		{
		...
		if(joint == SkeletonJoint.LeftShoulder)
		{
	...
	new public void RotateToCalibrationPose()
	{
		foreach (SkeletonJoint j in Enum.GetValues(typeof(SkeletonJoint)))
		{
		...
		if (null != RightShoulder) {
			RightShoulder.rotation = transform.rotation * Quaternion.Euler(0, 0, -90) * initialRotations[(int)IxoSkeletonJoint.RightShoulder];
		}
		if (null != LeftShoulder) {
        	LeftShoulder.rotation = transform.rotation * Quaternion.Euler(0, 0, 90) * initialRotations[(int)IxoSkeletonJoint.LeftShoulder];
		}

...
public class PositionTweenProperty : AbstractVector3TweenProperty
{
	...
	public override void prepareForUse()
	{
		...
		{
			_startValue = !_isRelative ? _endValue : -_endValue + (_useLocalPosition ? _target.localPosition : _target.position);
		...
		{
			_startValue = _useLocalPosition ? _target.localPosition : _target.position;
		}
		*/
		...
		if( _ownerTween.isFrom )
		{
			...
			_startValue = _isRelative ? _endValue + (_useLocalPosition ? _target.localPosition : _target.position) : _endValue;

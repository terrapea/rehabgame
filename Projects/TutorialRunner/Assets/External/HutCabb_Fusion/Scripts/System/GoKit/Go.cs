...
public class Go : MonoBehaviour
{
	...
	{
#if UNITY_EDITOR
		__numTweensHeld = _tweens.Count;
		
		if(Input.GetKeyDown(KeyCode.F10))
		{
			for(int i = 0; i < _tweens.Count; i++)
			{
				List<AbstractTweenProperty> list = _tweens[i].allTweenProperties();
				print(_tweens[i].autoRemoveOnComplete);
				print(_tweens[i].duration);
				print(_tweens[i].loopType);
				print(_tweens[i].state);
				print(_tweens[i].totalElapsedTime);
				print(_tweens[i].GetType());
				Tween t = _tweens[i] as Tween;
				if(t != null)
					print(t.target);
			
				for(int j = 0; j < list.Count; j++)
				{
					print(list[j].GetType());
				}
			}
		}
#endif

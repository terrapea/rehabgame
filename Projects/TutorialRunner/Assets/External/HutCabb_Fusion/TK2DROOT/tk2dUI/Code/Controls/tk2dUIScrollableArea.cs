...
public class tk2dUIScrollableArea : MonoBehaviour
{
    ...
    void OnDisable()
    {
        ...
        if (isBackgroundButtonOver)
        {
            ...
            {
                tk2dUIManager.Instance.OnMouseWheelChange -= BackgroundHoverOverMouseWheelChange;
            }
    ...
    private void BackgroundHoverOverMouseWheelChange(float mouseWheelChange)
    {
    ...
    {
        Vector3 localPos = contentContainer.transform.localPosition;
        ...
        contentContainer.transform.localPosition = localPos;
    }
    ...
    private void BackgroundButtonDown()
    {
        if (allowSwipeScrolling && contentLength > visibleAreaLength)
        {
            ...
            swipeScrollingContentStartLocalPos = contentContainer.transform.localPosition;
    ...
    private void BackgroundOverUpdate()
    {
        ...
        if (isSwipeScrollingInProgress)
        {
            ...
            if (isBackgroundButtonDown)
            {
                ...
                contentContainer.transform.localPosition = swipeScrollingContentDestLocalPos;
            }
            ...
            if (scrollBar != null)
            {
                ...
                if (scrollAxes == Axes.XAxis)
                {
                    scrollBarPercent = (contentContainer.transform.localPosition.x / (contentLength - visibleAreaLength));
                }
                else if (scrollAxes == Axes.YAxis)
                {
                    scrollBarPercent = (contentContainer.transform.localPosition.y / (contentLength - visibleAreaLength));
    ...
    private void UpdateSwipeScrollDestintationPosition()
    {
        ...
        if (isSwipeScrollingInProgress)
        {
            ...
            destContentPos.z = contentContainer.transform.localPosition.z;

            if (scrollAxes == Axes.XAxis)
            {
                destContentPos.y = contentContainer.transform.localPosition.y;        
            }
            else if (scrollAxes == Axes.YAxis)
            {
                destContentPos.x = contentContainer.transform.localPosition.x;
            }
            destContentPos.z = contentContainer.transform.localPosition.z;
